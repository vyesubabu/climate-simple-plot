...
def printstat(data, header):
    mean = round(np.mean(data), 3)
    max_ = round(np.amax(data), 3)
    min_ = round(np.amin(data), 3)
...
printstat(annual_data[index], 'Annual TAVG')
...
printstat(month_data[index], 'Monthly TMAX')
